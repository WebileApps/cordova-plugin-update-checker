...
- (void)checkForUpdate {
    ...
    [[session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (error) {
            NSLog(@"Error checking for updates: %@", error.localizedDescription);
            return;
        }

        NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)response;
        NSString *lastModifiedString = [httpResponse allHeaderFields][@"Last-Modified"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"EEE, dd MMM yyyy HH:mm:ss zzz"];
        NSDate *lastModifiedDate = [dateFormatter dateFromString:lastModifiedString];
        ...
        long storedTimestamp = [[defaults stringForKey:@"lastModified"] longLongValue];
        NSLog(@"Stored last modified time: %ld", storedTimestamp);

        if (self.lastModified > storedTimestamp) {
...
- (void)showUpdateDialog {
    ...
    UIAlertAction *reloadAction = [UIAlertAction actionWithTitle:@"Reload"
        ...
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:[NSString stringWithFormat:@"%ld", self.lastModified] forKey:@"lastModified"];
...
- (void)reloadWebView {
    [(UIWebView *)self.webViewEngine.engineWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.launchUrl]]];
}
